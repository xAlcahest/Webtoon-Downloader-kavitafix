...
from webtoon_downloader.core.webtoon.season_parser import generate_kavita_filename
...
@dataclass
class KavitaFileNameGenerator(FileNameGenerator):
    """
    Kavita-compatible file name generator that uses the Tranga-style naming convention.
    
    Generates file names in the format: "Series Name Vol.X Ch.Y - Chapter Title"
    where Volume corresponds to Season and Chapter corresponds to Episode.
    """
    ...
    def get_chapter_directory(self, chapter_info: ChapterInfo) -> Path:
        """
        Returns the directory path for storing the given chapter's data.
        Uses Kavita-compatible naming with volume and chapter information.
        """
        if self.use_chapter_title_directories:
            return Path(sanitize_filename(chapter_info.title))

        # Use Kavita-style directory name
        kavita_name = generate_kavita_filename(
            chapter_info.series_title,
            chapter_info.volume_number,
            chapter_info.number,
            chapter_info.title
        )
        return Path(sanitize_filename(kavita_name))
...
class KavitaNonSeparateFileNameGenerator(FileNameGenerator):
    """
    Kavita-compatible file name generator for when chapters and pages are stored in the same directory.
    """
    ...
    def get_page_filename(self, page_info: PageInfo) -> str:
        """
        Generates a Kavita-compatible file name for a page.
        Format: "Series Name Vol.X Ch.Y - Chapter Title_PageNumber.extension"
        """
        chapter_info = page_info.chapter_info
        kavita_base = generate_kavita_filename(
            chapter_info.series_title,
            chapter_info.volume_number,
            chapter_info.number,
            chapter_info.title
        ...
        return sanitize_filename(f"{kavita_base}_{page_number}.{extension}")
    ...
    def get_title_filename(self, chapter_info: ChapterInfo) -> str:
        ...
        return sanitize_filename(f"{kavita_base}_title.txt")
    ...
    def get_notes_filename(self, chapter_info: ChapterInfo) -> str:
        ...
        return sanitize_filename(f"{kavita_base}_notes.txt")
